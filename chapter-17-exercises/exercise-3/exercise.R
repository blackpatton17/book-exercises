...
populations <- read.csv("data/uscitiesv1.4.csv", stringsAsFactors = F)
...
map <- leaflet(most_populous) %>% addTiles()
...
map <- map %>% addCircleMarkers(
  lat = most_populous$lat,
  lng = most_populous$lng,
  popup = most_populous$city,
  stroke = FALSE,
  radius = 1,
  fillOpacity = .5
)
